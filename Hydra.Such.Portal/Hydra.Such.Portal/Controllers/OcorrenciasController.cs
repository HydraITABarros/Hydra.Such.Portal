    ...
    public class OcorrenciasController : Controller
    {
        ...
        public async Task<JsonResult> ExportToExcel_Ocorrencias([FromBody] List<OcorrenciasViewModel> Lista)
        {
            ...
            using (var fs = new FileStream(Path.Combine(sWebRootFolder, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                if (dp["motivo"]["hidden"].ToString() == "False") { row.CreateCell(Col).SetCellValue("Motivo"); Col = Col + 1; }
                ...
                if (dp != null)
                {
                    ...
                    foreach (OcorrenciasViewModel item in Lista)
                    {
                        ...
                        if (dp["motivo"]["hidden"].ToString() == "False") { row.CreateCell(Col).SetCellValue(item.Motivo); Col = Col + 1; }
        ...
        public JsonResult CreateMedidaCorretiva([FromBody] JObject requestParams)
        {
            ...
            int codOcorrencia = int.Parse(requestParams["codOcorrencia"].ToString());
        ...
        public JsonResult GetOcorrenciaDetails([FromBody] OcorrenciasViewModel data)
        {
            ...
            if (data != null && data.CodOcorrencia > 0)
            {
        ...
        public JsonResult CreateOcorrencia([FromBody] OcorrenciasViewModel data)
        {
            if (data != null && data.CodOcorrencia == 0)
            {
                ...
                {
                    data.eReasonCode = 1;
                    data.eMessage = "Ocorrência criada com sucesso.";
                }
                else
                {
                    data.eReasonCode = 2;
        ...
        {
            if (data != null && !string.IsNullOrEmpty(data.CodEncomenda))
            {
                ...
                if (Encomenda != null)
                {
                    ...
                    data.LocalEntrega = Encomenda.PayToCity;
                    //data.NoDocExterno
                    data.CodArtigo = Encomenda.NoConsulta;
                    data.Descricao = Encomenda.PostingDescription;
                    //data.UnidMedida
                    //data.Quantidade = Encomenda
                }
            }
