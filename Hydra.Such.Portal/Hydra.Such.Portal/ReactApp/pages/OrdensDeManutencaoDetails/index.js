...
class OrdensDeManutencaoLine extends Component {
	...
	fetchNext = (skip, take) => {
		if (this.state.maintenanceOrdersLinesNext == "") {
			return this.fetchmaintenanceOrdersLines({ orderId: this.state.orderId }, () => { });
		}
		if (this.state.maintenanceOrdersLinesNext && this.state.maintenanceOrdersLinesNext != "" &&
			!this.state.maintenanceOrdersLinesIsLoading &&
			!this.state.isLoading && this.state.maintenanceOrdersTotal > this.state.maintenanceOrdersLines.length) {
			this.setState({ maintenanceOrdersLinesIsLoading: true }, () => {

				axios.get(this.state.maintenanceOrdersLinesNext).then((result) => {
					...
					if (data.ordersCountsLines && data.resultLines && data.resultLines.items) {
						...
						this.setState({
							...
							this.setState({
								...
								if (this.state.maintenanceOrdersTotal == this.state.maintenanceOrdersLines.length) {
	...
	render() {
		...
		console.log(getDefaultExpandedGroups(this.state.maintenanceOrdersLines, this.state.group));
				...
				{this.state.listContainerStyle.marginTop && !isLoading ?
					<ListContainer ref={el => this.listContainer = el} style={{ ...this.state.listContainerStyle }} onScroll={this.handleGridScroll} >
						...
						>
							...
							<SelectionState />
							<GroupingState
								expandedGroups={getDefaultExpandedGroups(this.state.maintenanceOrdersLines, this.state.group)}
								grouping={this.state.group}
							...
							<VirtualTableState
								...
								totalRowCount={this.state.maintenanceOrdersTotal + this.state.defaultExpandedGroups.length}
								pageSize={30}
							...
							<VirtualTable
								...
								_containerComponent={(props) => {
									var onScroll = (e) => {
										console.log(e.currentTarget.scrollTop + e.currentTarget.offsetHeight + 1000,
											(this.state.maintenanceOrdersLines.length + this.state.groupingStateColumnExtensions.length) * 56
										);

										if (e.currentTarget.scrollTop + e.currentTarget.offsetHeight + 200 > (this.state.maintenanceOrdersLines.length) * 56) {
									...
									return (<VirtualTable.Container {...props} onScroll={onScroll} />);
								}}

								_tableComponent={props => {
									return (<VirtualTable.Table {...props} onScroll={console.log} />);
								}}

								_stubRowComponent={(props) => {

									var keys = props.tableRow.key.split('_');
									var total = keys[2];
									var current = keys[1];

									// props.tableRow.height = ((total * 1) - (current * 1)) * 56;
									// props.style.height = props.tableRow.height + "px";

									// if (keys[1] == keys[2]) {
									//         props.style.height = "0px";
									//         props.tableRow.height = 0;
									// } else {
									//         props.tableRow.height = props.tableRow.height - 56;
									//         props.style.height = props.tableRow.height + "px";
									// }
									//var rowHeight = 56;
									//props.style.height = ((this.state.maintenanceOrdersLines.length * 1) - (this.state.maintenanceOrdersTotal) * rowHeight + 'px');
