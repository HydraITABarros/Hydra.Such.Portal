...
const AvatarsType = (props) => {
	...
	return (
		<AvatarGroup style={{ top: "-7px", position: "relative", height: '1px', fontSize: '1px' }} >
			{props.value && props.value.map((_item, i) => (<Avatars.Avatars key={i} letter color={avatarColors[i]} data-tip={_item.nome} >{functions.getInitials(_item.nome)}</Avatars.Avatars>))}
		</AvatarGroup>
	)
};
