using System;
using System.Collections.Generic;

namespace Hydra.Such.Data.ViewModel.FH
{
    public partial class AutorizacaoFHRHViewModel
    {
        public string NoEmpregado { get; set; }
        public string NomeEmpregado { get; set; }
        public string NoResponsavel1 { get; set; }
        public string NomeResponsavel1 { get; set; }
        public string NoResponsavel2 { get; set; }
        public string NomeResponsavel2 { get; set; }
        public string NoResponsavel3 { get; set; }
        public string NomeResponsavel3 { get; set; }
        public string ValidadorRH1 { get; set; }
        public string NomeValidadorRH1 { get; set; }
        public string ValidadorRH2 { get; set; }
        public string NomeValidadorRH2 { get; set; }
        public string ValidadorRH3 { get; set; }
        public string NomeValidadorRH3 { get; set; }
        public string ValidadorRHKM1 { get; set; }
        public string NomeValidadorRHKM1 { get; set; }
        public string ValidadorRHKM2 { get; set; }
        public string NomeValidadorRHKM2 { get; set; }
        public string UtilizadorCriacao { get; set; }
        public DateTime? DataHoraCriacao { get; set; }
        public string UtilizadorModificacao { get; set; }
        public DateTime? DataHoraModificacao { get; set; }
    }
}
