    ...
    public static class DBProjectDiary
    {
        ...
        public static ProjectDiaryViewModel ParseToViewModel(DiárioDeProjeto x)
        {
            return new ProjectDiaryViewModel()
            {
                ...
                TotalCost = x.Quantidade.HasValue && x.CustoUnitário.HasValue ? Math.Round((decimal)(Math.Round((decimal)x.Quantidade, 2) * Math.Round((decimal)x.CustoUnitário, 4)), 2) : decimal.Zero,
                UnitPrice = x.PreçoUnitário.HasValue ? Math.Round((decimal)x.PreçoUnitário, 4) : decimal.Zero,
                TotalPrice = x.Quantidade.HasValue && x.PreçoUnitário.HasValue ? Math.Round((decimal)(Math.Round((decimal)x.Quantidade, 2) * Math.Round((decimal)x.PreçoUnitário, 4)), 2) : decimal.Zero,
