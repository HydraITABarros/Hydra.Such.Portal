    ...
    public static class DBProjectDiary
    {
        ...
        public static ProjectDiaryViewModel ParseToViewModel(DiárioDeProjeto x)
        {
            return new ProjectDiaryViewModel()
            {
                ...
                TotalCost = x.Quantidade.HasValue && x.CustoUnitário.HasValue ? Math.Round((decimal)(x.Quantidade * x.CustoUnitário), 2) : decimal.Zero,
                UnitPrice = x.PreçoUnitário.HasValue ? Math.Round((decimal)x.PreçoUnitário, 4) : decimal.Zero,
                TotalPrice = x.Quantidade.HasValue && x.PreçoUnitário.HasValue ? Math.Round((decimal)(x.Quantidade * x.PreçoUnitário), 2) : decimal.Zero,
