...
namespace Hydra.Such.Data.Logic.Contracts
{
    public static class DBContractsEstadoAlteracao
    {
        ...
        public static ContratosEstadoAlteracao ParseToDB(Contratos x)
        {
            ...
            ContratosEstadoAlteracao result = new ContratosEstadoAlteracao()
            {
                ...
                NºDeContrato = x.NºDeContrato,
                NºVersão = x.NºVersão,
                Área = x.Área,
                Descrição = x.Descrição,
                Estado = x.Estado,
                EstadoAnterior = x.EstadoAnterior,
                EstadoAlteração = x.EstadoAlteração,
                NºCliente = x.NºCliente,
                ...
                CódEndereçoEnvio = x.CódEndereçoEnvio,
                EnvioANome = x.EnvioANome,
                EnvioANome2 = x.EnvioANome2,
                EnvioAEndereço = x.EnvioAEndereço,
                EnvioAEndereço2 = x.EnvioAEndereço2,
                EnvioACódPostal = x.EnvioACódPostal,
                EnvioALocalidade = x.EnvioALocalidade,
                PeríodoFatura = x.PeríodoFatura,
                ÚltimaDataFatura = x.ÚltimaDataFatura,
                PróximaDataFatura = x.PróximaDataFatura,
                DataInicial = x.DataInicial,
                DataExpiração = x.DataExpiração,
                JuntarFaturas = x.JuntarFaturas,
                PróximoPeríodoFact = x.PróximoPeríodoFact,
                LinhasContratoEmFact = x.LinhasContratoEmFact,
                CódTermosPagamento = x.CódTermosPagamento,
                CódFormaPagamento = x.CódFormaPagamento,
                TipoProposta = x.TipoProposta,
                TipoFaturação = x.TipoFaturação,
                TipoContratoManut = x.TipoContratoManut,
                NºRequisiçãoDoCliente = x.NºRequisiçãoDoCliente,
                DataReceçãoRequisição = x.DataReceçãoRequisição,
                NºCompromisso = x.NºCompromisso,
                TaxaAprovisionamento = x.TaxaAprovisionamento,
                Mc = x.Mc,
                TaxaDeslocação = x.TaxaDeslocação,
                ContratoAvençaFixa = x.ContratoAvençaFixa,
                ObjetoServiço = x.ObjetoServiço,
                ContratoAvençaVariável = x.ContratoAvençaVariável,
                Notas = x.Notas,
                DataInícioContrato = x.DataInícioContrato,
                DataFimContrato = x.DataFimContrato,
                DescriçãoDuraçãoContrato = x.DescriçãoDuraçãoContrato,
                DataInício1ºContrato = x.DataInício1ºContrato,
                Referência1ºContrato = x.Referência1ºContrato,
                DuraçãoMáxContrato = x.DuraçãoMáxContrato,
                RescisãoPrazoAviso = x.RescisãoPrazoAviso,
                CondiçõesParaRenovação = x.CondiçõesParaRenovação,
                CondiçõesRenovaçãoOutra = x.CondiçõesRenovaçãoOutra,
                CondiçõesPagamento = x.CondiçõesPagamento,
                CondiçõesPagamentoOutra = x.CondiçõesPagamentoOutra,
                AssinadoPeloCliente = x.AssinadoPeloCliente,
                Juros = x.Juros,
                DataDaAssinatura = x.DataDaAssinatura,
                DataEnvioCliente = x.DataEnvioCliente,
                UnidadePrestação = x.UnidadePrestação,
                ReferênciaContrato = x.ReferênciaContrato,
                ValorTotalProposta = x.ValorTotalProposta,
                LocalArquivoFísico = x.LocalArquivoFísico,
                NºOportunidade = x.NºOportunidade,
                NºProposta = x.NºProposta,
                NºContato = x.NºContato,
                DataEstadoProposta = x.DataEstadoProposta,
                OrigemDoPedido = x.OrigemDoPedido,
                DescOrigemDoPedido = x.DescOrigemDoPedido,
                NumeraçãoInterna = x.NumeraçãoInterna,
                DataAlteraçãoProposta = x.DataAlteraçãoProposta,
                DataHoraLimiteEsclarecimentos = x.DataHoraLimiteEsclarecimentos,
                DataHoraErrosEOmissões = x.DataHoraErrosEOmissões,
                DataHoraRelatórioFinal = x.DataHoraRelatórioFinal,
                DataHoraHabilitaçãoDocumental = x.DataHoraHabilitaçãoDocumental,
                DataHoraEntregaProposta = x.DataHoraEntregaProposta,
                NºComprimissoObrigatório = x.NºComprimissoObrigatório,
                ...
                RazãoArquivo = x.RazãoArquivo,
                ValorBaseProcedimento = x.ValorBaseProcedimento,
                AudiênciaPrévia = x.AudiênciaPrévia,
