...
namespace Hydra.Such.Data.Logic
{
    ...
    {
        #region CRUD
        public static QuestionarioFornecedorGestaoAmbiental GetById(int ID)
        {
            try
            {
                using (var ctx = new SuchDBContext())
                {
                    return ctx.QuestionarioFornecedorGestaoAmbiental.Where(x => x.ID == ID).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
        ...
        public static bool Delete(int ID)
        {
            try
            {
                using (var ctx = new SuchDBContext())
                {
                    QuestionarioFornecedorGestaoAmbiental Questionario = ctx.QuestionarioFornecedorGestaoAmbiental.Where(x => x.ID == ID).FirstOrDefault();
                    if (Questionario != null)
                    {
                        ctx.QuestionarioFornecedorGestaoAmbiental.Remove(Questionario);
                        ctx.SaveChanges();
                        return true;
                    }
                }
